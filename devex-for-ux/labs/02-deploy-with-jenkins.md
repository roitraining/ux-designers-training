...
Once you refresh the Source Repos page, you should see your README file. It is a time-honored tradition to have a README.md file at the root of all Git repos to explain what is in the repo, how to get it running, and usually something about the tools that are used.

### Create the Sample Application
First, create a text file called '.gitignore' and write node_modules inside it. This tells Git to ignore your node_modules folder which includes your dependencies. We don't want to continually transfer the dependencies around with Git, those should come directly from NPM. You can read more about best practices for dependencies in the [corresponding twelve-factor best practice](https://12factor.net/dependencies).

Now you are ready to create your app. Since you are in Cloud Shell, Node and NPM have already been installed for you. You can begin by going through the [Express Installation](https://expressjs.com/en/starter/installing.html). Follow the link at the bottom to get the Hello World example we used previously. Make sure that when you install Express with NPM you do use the --save flag. This will make sure it's included as a dependency in your package.json file.
...
To read more on storing config in the environment, go to [the twelve-factor best practice](https://12factor.net/config).
...
### Prepare the VM
SSH into your new VM, jenkins-example. Because we aren't in Cloud Shell, we'll need to install the tools needed.
...
Install gcloud by following [the official documentation](https://cloud.google.com/sdk/docs/quickstart-debian-ubuntu). You should not need to init gcloud because your Compute Engine VM comes with a default service account.


### Clone the Repo in the VM
Go back to Source Repos, click the **Clone** button, choose **gcloud**, and then follow the steps. You will get an error that you have insufficient scopes. To fix that:
1. Stop the VM.
...
Press CTRL-C to end the process, then run the following to install a Process Management tool called PM2:
``` bash
...
If you named your file something other than app.js, use that instead. And remember that we are using sudo because we are running the app directly on port 80. That is not how it's done in the real world, but avoids a lot of complex configuration.

**Bonue:** If you want to try running it properly, follow this commonly used [guide](https://www.digitalocean.com/community/tutorials/how-to-set-up-a-node-js-application-for-production-on-debian-9) from Digital Ocean. It will cover a more robust use of PM2, as well as using Nginx as a reverse proxy. Notice in their prereqs they cover how to install Nginx; you'll have to do that too.
...
### Make a Change
Go back to Cloud Shell and make a change to your main app file, app.js or whatever you named it (from here on out I'll stop calling this out specifically). Change "Hello World" to some other message. If you want to get fancy, you could even add some HTML and CSS.
...
Refresh the browser where you are viewing the web app, and notice that it has not changed. When you change server-side code, you need to start the process. Go back to your SSH window and run:
```
pm2 restart app
```
...
**Bonue:** Release a change through a different branch.
...
This is closer to real world, however in the real world it is typical to submit pull requests and have someone else review your code before merging.
...
3. Apply this rule to the VM by clicking it in the GUI, editing it, and then under networking, adding the tag allow-8080.
...
2. Once Jenkins is running, leave that terminal window running and open a new terminal window to work from. Click **SSH** in the Compute Engine GUI again.
...
From the Google Cloud Console, go to **IAM > Service Accounts**, and find your **Compute Engine Default Service Account**. Click it, edit it, and create a new key. Download the JSON key file. We'll use this to authenticate in a moment.
...
Back in the Jenkins Dashboard, click **New Item**. Give it a name and choose **Freestyle Project** and the click **Okay**. Under Source Code Management, select **Git**.
...
From **Cloud Source Repositories**, go into your sample-app repository, and click **Clone**. Switch to **Manually generated credentials** and copy the URL that comes after git clone. Back in Jenkins, paste this into the **Repository URL**.
...
**Important Notes**
1. Notice you are telling Jenkins to run the same set of commands you would normally do manually to deploy a new version.
