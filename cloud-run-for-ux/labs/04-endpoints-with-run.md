...
### Deploying ESPv2 Beta to Cloud Run
...
esp-gateway-blahblah-uc.a.run.app (notice, no https, just the host). Then replace the address with the address for your events service. It will look something like:
...
5. Switch to your Cloud Shell window and `Open Editor` using the button that reads the same or looks like a pencil. 
...
Still following Google's example, we are now in the section: 
...
3. Upload the configuration and create the managed service. Durring the configuration process you will see several messages. One warns that currently there is no key required for our service. In real life we would want to turn on some sort of API security. 

```
gcloud endpoints services deploy events-api.yaml
```

4. In the messages after the last command, locate the one that uses the format `Service Configuration [Your config ID] uploaded for service [host name]`. Take note of the config ID, you'll need it in a few steps. 
...
3. Run the script to build the service into a new container image, which gets uploaded to the Container Registry. Make sure to specify the hostname and config id you noted at the end of the last section. 
...
4. When the image build completes, switch to the Google Cloud Console and look at the images in Container Registry. You should see a just deployed "endpoints-runtime-serverless". Hover over the image's name a copy button should appear with the tool tip "Copy full image name." Copy the full name and drop it in a text document or something, you'll need it in the next step. 
...
1. Look at the *events-service/routes.js* file and it will show you all the paths along with the http verb (get, post, delete, put, patch) used with each one. You will probably have to take a peak at the code and/or, invoke the methods in the browser to figure out how each one works, and how to translate that to paths and schemas in the OpenAPI file.  
...
Tip: take a peak at the example in the editor when you first pull it up. It has a lot of good examples. I like to open it in second tab so I can look to it for advice where needed. 
...
1. Use the Cloud Run page in the Google Cloud console to create a new revision for your web-app service. Edit the services variables and update the path to the events service so that it runs through your Cloud Endpoint. 

As a side note, if you also updated your users service to run through the gateway, the base URLs for both the services would be the same. One would be /events and one would /users. Service consumers wouldn't need to even know that they were actually different services. Another nice Cloud Endpoints feature. 
