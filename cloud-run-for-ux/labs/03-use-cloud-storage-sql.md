...
- To interact with Cloud Storage from a VM
...
1. Log into your class account and project. `Activate Cloud Shell` using the icon near the upper right corner, and run the following command to create a new VM named storage-example:
...
8. Install the light weight Node.js Express web server and framework. This command adds the dependency to package.json, and downloads the dependency, and all its dependencies, into a node_modules folder:
...
11. Exit Nano by clicking `ctrl-x` and answering `Y` and `Enter` to save the modified buffer.
...
3. Set the bucket's location type to `Region` and set the bucket's location to `us-central1`, then `Continue`. 

4. Leave the storage class `Standard`, scroll to the bottom of the page, then `Create` the bucket. 

6. Make sure to record the name of your bucket, you'll need that in a bit. 


### Install the Cloud Storage Client Library for Node.js

1. Switch back to the SSH window for your VM. It should still be running your web application. Enter `ctrl-c` to stop its execution. 
...
4. Edit '[BUCKET_NAME]' and replace it with your bucket name prefaced by "gs://", like: 'gs://your-bucket-name-here'
...
6. If you get any errors, fix them. If it starts normally, stop the app again by executing `ctrl-c`.
...
```
**Info** A "route" in this context is a URL pattern your web application will respond to. So, if someone visits your app's url/, they get the Hello World message. With the above new code, if the visit your app's url/add-file, they get the ability to upload a file. 
...
4. Visit your application again using its external IP. Remember, that IP is easily found on the Compute Engine page. Once you receive the "Hello World!" message, edit the URL by adding `/add-file?fileName=test.txt&fileContents=Hello%20World` to the end. You should end up with a URL something like: http://[YOUR_VMS_IP]/add-file?fileName=test.txt&fileContents=Hello%20World
...
1. In your VM's SSH window, use `ctrl-c` to stop the running application. Then, open your *index.js* file in Nano. Just below the line where you create *bucket*, insert the following line to load *fs*.

``` javascript
const fs = require('fs')
```

2. Locate your 'add-file' controller method, add the following just above "res.send('Filename...')":
...
4. Switch back to the SSH window. Stop the application with `crtl-c` and do a directory listing of all your files. 

```
ls
```

5. You should see a new file created named *test.txt* (or whatever you specified in the fileName part of the URL). List the files contents and you should see the value from the fileContents query string. 
...
A nice download files example can be found here?:
https://cloud.google.com/storage/docs/downloading-objects#storage-download-object-nodejs 

1. If you have your web application running, use `ctrl-x` to stop it. Also, verify that your command prompt is currently in the my-app folder of your VM SSH window.
...
10. Save your code and re-run the application. Make sure to test it on the file you stored in Cloud Storage in the last section of the exercise. If all went well, you should be able to use your /read-file endpoint to get the contents of a file on Cloud Storage.
