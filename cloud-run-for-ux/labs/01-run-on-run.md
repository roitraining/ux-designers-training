...
We are going to continue to use the sample application from our last class, Slick Tickets. Let's start by getting signed into Google Cloud, make sure a few required services are active, clone the Slick Tickets Git repository, and build the application containers. 

1. Log into [Google Cloud](https://console.cloud.google.com/) using the trial account you created for this course. You should not be using your actual work Google account. If you do, you will have security issues. 
...
4. We need to activate Firestore so it will be ready to store data when we run our application. Let's use Native mode by clicking `Select Native Mode`. Set the region to `nam5 (United States)` and `Create Database`.
...
13. Switch to the Google Cloud Consol in your browser and use the hamburger menu to navigate to the Container Registry (under Tools). Verify your three service image folders and if you like, drill down into them and investigate the images themselves. 
...
9. ***web-app only*** Using the link at the top of the page, switch to `Variables`. Using the links you coppied for the other two services, add the following variables (don't enter any port numbers):  
...
13. From the Slick Ticket's home page, follow the link to `Login` and Register a new account. Use your account to log into the application, and make sure not to forget your credentials! 
...
1. Switch back to Cloud Shell and `Open Editor`. 
...
4. Switch from the editor to the Cloud Shell terminal, it will auto save the file. 
...
7. Click `web-app` to view its details, then click `Edit & Deploy New Revision`. Changing the container image, or any of the other variables or settings, is accomplished by creating a new revision of the same service. 
...
5. After it has completed and shows that it is serving 100% of traffic to your new revision, check it out in the browser to make sure you see your new change. Remember, the title is just the label on the tab at the top of the page, it doesn't appear in the page text.  
...
1. Switch to the Google Cloud console and using the hamburger menu to navigate to `Firestore`.
